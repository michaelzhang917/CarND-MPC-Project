...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  ret_vector.push_back(solution.x[delta_start]);
  ret_vector.push_back(solution.x[a_start]);
  for (int i = 0; i < N-1; i++){
    ret_vector.push_back(solution.x[x_start + i + 1]);
    ret_vector.push_back(solution.x[y_start + i + 1]);
  }
